...
def train_classifier(dataset, features, labels, logits, optimiser, minibatch_size, num_epochs):
    accuracy = ((lambda xs: (tf.reduce_mean)((tf.to_float)((tf.equal)(tf.argmax(xs, axis=1), tf.argmax(labels, axis=1)))))(logits))
    cross_entropy = ((lambda ls: (tf.reduce_mean)(tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels, logits=ls)))(logits))
    ...
    def train_one_epoch(session):
        train_loss = ((lambda train: (np.mean)((list)(map(lambda pair: train_one_step(session, *pair), split(train['features'], train['labels'])))))(dataset['train']))
    ...
    with tf.Session() as session:
        ...
        ((list)(map(print, map(lambda _: train_one_epoch(session), range(0, num_epochs)))))
    return session

...
def reshape_to_matrix(xs):
    n_cols = reduce(_coconut.operator.mul, map(_coconut.operator.attrgetter("value"), xs[0].shape))
    return _coconut_tail_call(tf.reshape, xs, [-1, n_cols])

if __name__ == '__main__':
    features = tf.placeholder('float', [None, 784])
    labels = tf.placeholder('float', [None, 10])

    conv = lambda xs, fsize, ksize: tf.layers.conv2d(xs, fsize, ksize, padding='same', activation=tf.nn.relu)
    pool = lambda xs, psize, stride: tf.layers.max_pooling2d(xs, psize, stride)
    dense = lambda xs, units: tf.layers.dense(xs, units, activation=tf.nn.relu)
    dropout = tf.layers.dropout
    logits = ((lambda xs: (lambda xs: (lambda xs: (lambda xs: (lambda xs: (lambda xs: (tf.nn.softmax)(dense(xs, 10)))(dropout(xs, 0.4)))(dense(xs, 100)))((reshape_to_matrix)(pool(xs, [2, 2], 2))))(conv(xs, 32, [5, 5])))(tf.reshape(xs, [-1, 28, 28, 1])))(features))

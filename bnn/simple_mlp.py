...
# Computation graph
    weights = {'hidden': tf.Variable(tf.random_normal([784, 100])), 'output': tf.Variable(tf.random_normal([100, 10]))}
    biases = {'hidden': tf.Variable(tf.random_normal([100])), 'output': tf.Variable(tf.random_normal([10]))}
    logits = ((lambda xs: (lambda hs: ((tf.matmul)(hs, weights['output'])) + biases['output'])(((tf.matmul)(xs, weights['hidden'])) + biases['hidden']))(features))
